...
**SE and NO fallback sources implemented** (`fetch_nordpool_prices`): single dispatcher covering both elprisetjustnu.se (SE1–SE4) and hvakosterstrommen.no (NO1–NO5). Confirmed from live API responses:

- SE: native 15-min resolution (96 slots/day). Fields: `EUR_per_kWh`, `time_start`, `time_end`.
- NO: hourly resolution (24 slots/day), expanded to 4×15-min at the same price. Same fields.

Both return EUR/kWh ex-VAT sourced from ENTSO-E. SEK/NOK fields present in response but unused — consistent with the rest of the pipeline which is EUR throughout. ENTSO-E also returns EUR/MWh for SE and NO areas.

Fetches yesterday, today, and tomorrow separately (one calendar day per request). Tomorrow returns 404 until Nord Pool publishes (~13:00 local) — handled silently, `PricesNotYetAvailable` raised if no future slots result.
...
**`_SE_AREAS` and `_NO_AREAS`** added to area sets, including full EIC codes. `_NORDPOOL_AREAS = _SE_AREAS | _NO_AREAS` used in `_build_fallback_chain`.

**`_SOURCE_NAMES`** updated with `"fetch_nordpool_prices": "nordpool-regional"`.

**6 new tests** in `TestBuildFallbackChain` (chain order for SE1/SE4/NO1/NO5, EIC code equivalence, no Elering in SE/NO) and 8 new tests in `TestAreaFallbackChainIntegration` (ENTSO-E success, nordpool tried when ENTSO-E fails, Elering/Sähkötin never called, exit when both fail — for SE1 and NO1).

**216 tests** passing, 3 skipped.

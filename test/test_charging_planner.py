...
class TestNtfyMessage(unittest.TestCase):
    """Tests for ntfy message building and send_ntfy config handling."""
    ...
    def test_send_ntfy_disabled_does_nothing(self):
        ...
        send_ntfy([self.PLAN], [], {"enabled": False, "topic": ""})

    def test_send_ntfy_no_topic_warns(self):
        from charging_planner import send_ntfy
        import unittest.mock as mock
        with mock.patch("charging_planner.log") as mock_log:
            send_ntfy([self.PLAN], [], {"enabled": True, "topic": ""})
            mock_log.warning.assert_called()

    def test_send_ntfy_topic_from_env(self):
        from charging_planner import send_ntfy
        import unittest.mock as mock
        with mock.patch.dict("os.environ", {"NTFY_TOPIC": "test-topic"}):
            with mock.patch("urllib.request.urlopen") as mock_open:
                ...
                self.assertIn("test-topic", req.full_url)

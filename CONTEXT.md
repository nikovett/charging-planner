...
**The dependency does run one way, though**: `delivery/deliver.py` imports `charging_planner.py` directly (`sys.path.insert` to the repo root, then `import charging_planner as cp`), specifically for `cp.load_config`/`cp.ConfigError` — config-loading logic, not planning logic, so this doesn't reintroduce the "delivery knows about planning internals" coupling the separation above protects against. This import exists to fix a real regression: `deliver.py` originally had its own, completely independent `load_config` — a bare `yaml.safe_load` with no call to `translate_config` at all. It worked fine until `config.yaml` moved to the new `profiles:` format (see "Config format translation" below); `charging_planner.py`'s own `load_config` translated correctly, but `deliver.py`'s separate copy never got the update, silently found zero delivery entries for every profile, and logged only a mild-sounding warning — no error, no traceback, plans built correctly and nothing ever delivered. Fixed by deleting the duplicate rather than patching it to also call `translate_config`, so there's structurally one `load_config` in the codebase, not two that can drift apart again — the same lesson as the `min_gap_minutes` regression earlier in this project's history, just at module-import scope instead of a repeated literal.

**Worth remembering for testing anything with its own `if __name__ == "__main__"` entry point**: this bug was invisible to every test that called `deliver`'s functions directly within an already-running Python process — those tests import `charging_planner` themselves and share that already-correct module, which is exactly what masked the divergence. It only reproduces when `deliver.py` is actually run as its own subprocess (`python delivery/deliver.py ...`, matching both the GHA workflow and manual command-line use), exercising its own `sys.path`/import resolution independently. `TestDeliverModuleConfigLoading.test_real_subprocess_finds_deliveries_with_new_format_config` in `test/test_deliver.py` does exactly that, and is the only test in the suite that runs `deliver.py` as a real subprocess rather than calling into it directly — confirmed by temporarily reintroducing the old duplicate `load_config` and re-running the test, which failed as expected.
...
**`_parse_entsoe_xml`'s own count-log was a related, separate redundancy**: its `"Fetched N price slots for {date}"` line and `fetch_entsoe_prices`' own `"Fetched N total slots (M future, last: ...)"` line right after it both reported the exact same N — parsing's own line demoted to `log.debug` (its only production call site already logs the more informative summary). Its `"for {date}"` wording was also independently misleading — `target_date` is only ever used for logging/error-message context, never for parsing math, and the actual data spans ENTSO-E's full multi-day request window (yesterday evening through the day after tomorrow), not the single day the wording implied — reworded to `"requested around {date}"` while fixing the level.
...
`cmd_plan` used to make the display-padding call unconditionally (`if all_prices:`, no check for whether forecast data had already been fetched this run), so on any run where the fallback genuinely triggered — the common case for a weekend `any`/`any` entry before Sunday's prices publish — it made a second, wholly redundant network call to the same endpoint for data it had already retrieved and discarded moments earlier. Fixed, covering both places forecast data can already be in hand:

- **Supplement branch ran** (`price_source` was real, coverage was short) — the full fallback result is kept in scope (`forecast_fallback_full`) and reused.
- **Forecast won the fallback chain directly as the primary source** (all real sources failed) — `all_prices` itself already *is* that same uncapped fetch; no separate variable needed, just recognizing `price_source == "forecast"` with no supplement result to reuse means `all_prices` is the thing to reuse instead. Filtering here must be `start > last_real_slot` alone, **not** also excluding anything already in `all_prices` — in this case the reused source *is* `all_prices`, so an "already present" check would incorrectly exclude everything.

The separate fetch only happens when neither case applies (real prices were sufficient on their own), which is also the only case it's actually needed. Same "never make an unnecessary API call" principle established for delivery handlers, just not originally applied to the planner's own price-fetching.

**`fetch_forecast_prices` itself also had internal redundancy**, independent of the display-fetch issue above: two log lines ("fetching...", "N usable slots (from H hours, ex-VAT)") describing its *own* raw fetch, sitting right next to the caller's "Supplemented with N forecast slots" line in the supplement case — two different counts (729 raw fetched vs. 680 actually used as supplement, in one real example) that read as confusingly-different restatements of the same fact rather than genuinely separate information. But the function is *also* called directly as a fallback-chain primary source, where no such caller-side line exists — there, its own logging is the only report at all, and needs to stay, consistent with how ENTSO-E/Elering/Sähkötin report their own fetch counts. Resolved with a `quiet` parameter: `False` (default) for the primary-source case, `True` at the supplement call site specifically.
...
**The decision log line** (`_log_target`, called once at whichever of the function's five return points actually fires) shows a day-range mirroring config.yaml's own `schedule:` day-key notation — `"targeting thu-fri window (2026-09-24) — 21:00–06:30 local, 210 min"` — computed from the *actual resolved* `ws`/`we`, not from the abstract candidate date. This matters: for an overnight window "for" Friday, `ws` falls on Thursday, so the range reads `thu-fri`, not just `fri` — and the logged date is `ws`'s date (matching `plan_date` and the plan JSON's own `"date"` field exactly, which the previous version of this log did not: it showed the target weekday's date, a different date than `plan_date` for every overnight case). A same-day window collapses to a single day (`thu`) rather than the redundant `thu-thu`. For `any`/`any`, the range can legitimately span the day the script ran *and* the target day (`fri-sat`, if run Friday afternoon for Saturday's `any`/`any` entry) — `any` means the window starts literally *now*, not on whatever day the schedule entry is nominally "for," unlike an overnight entry's deliberate evening-before anchor.

**`any_end_cap` (`min(last available price, plan_horizon)`) is correctly bound by whatever real price data actually exists, not by the generic `plan_horizon` ceiling** — verified with realistic price coverage (`test_any_end_bound_by_realistic_price_data_not_plan_horizon`), not just the `_far_future_prices()` fixture every other test in this class uses, which deliberately makes `plan_horizon` the binding constraint and never exercises the case where real data is the *more* restrictive bound. Real day-ahead prices only ever cover roughly today + tomorrow (published once daily); a Friday run targeting Saturday's `any`/`any` entry correctly stops at Saturday night, never reaching into Sunday just because `plan_horizon` alone would technically allow it.
...
## v2.1.0 — DRAFT (not yet released)
- **Breaking:** `config.yaml` format rewritten. Replaces `charging:` with `profiles:`; the old format is rejected outright, no backward-compatible fallback. `schedule:` replaces per-day list entries with day-range keys (`mon-fri`, `sat,sun`, `fri`, or a mix) mapping to `{ window, required }` — every day of the week must be covered by exactly one entry, checked at load time. `window` is one string (`HH:MM-HH:MM`, `any-any`, or mixed `any-HH:MM`/`HH:MM-any`) instead of separate start/end fields. `delivery:` (renamed from `deliveries:`) is a list of `{ handler_name: { ...params } }` entries with handler-specific key names (`vin` for MyŠkoda, `charger` for Charge Amps/Easee) replacing the generic `charge_point_id`. `area`/`timezone` are top-level — no more `entsoe:` block in the user-facing file. The `enabled` delivery key is removed (behaved identically to omitting the entry). Every profile setting other than `schedule:` is optional with a single documented default.
- **Fixed:** `min_gap_minutes` default was inconsistent — some code paths defaulted to 30 minutes, others to 15, depending on which path a profile went through. Now sourced from one place everywhere; the default is 15.
- **Improved:** clearer logs. One line now states the actual planning decision (which window, which day-range) instead of multiple lines describing candidates checked and discarded along the way. Four lines repeating numbers already shown elsewhere are now debug-only (`--debug`), not deleted. Spillover still logs at the normal level — it's not shown anywhere else.

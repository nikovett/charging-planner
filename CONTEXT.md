...
4. **`any` as a bare, unquoted YAML scalar is a real footgun** — the file's own comment on `preferred_window_start` already warns that HH:MM values must be quoted or misparse; `any` has the same class of risk and isn't visually distinguished from a quoted string value.
